...
Écrivez une fonction `celsius_vers_fahrenheit(celsius)` qui convertit une température de degrés Celsius en degrés
Fahrenheit.
...
Écrivez une fonction `hypotenuse(cote1, cote2)` qui calcule l'hypoténuse d'un triangle rectangle à partir des deux
côtés.
...
Écrivez une fonction `distance_deux_points(x1, y1, x2, y2)` qui calcule la distance entre deux points dans un plan
cartésien.
